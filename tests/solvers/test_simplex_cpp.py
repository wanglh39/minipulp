...
镜像 src/minipulp/solvers/simplex_cpp.py。
与 test_simplex_py.py 用相同用例，验证 C++ 核心与纯 Python 核心结果一致。
"""
...
def solve(prob):
    return prob.solve(solver=SimplexCpp())
...
class TestSolverInterface:
    ...
    def test_name(self):
        assert SimplexCpp().name == "SimplexCpp"
...
class TestCppPyConsistency:
    ...
    def test_consistency_diet(self):
        from minipulp.solvers import SimplexCore
        ...
        prob_py, x_py, y_py = build()
        prob_py.solve(solver=SimplexCore())

        prob_cpp, x_cpp, y_cpp = build()
        prob_cpp.solve(solver=SimplexCpp())
