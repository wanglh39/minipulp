...
用 Python 脚本写 C++ 文件，避免 write 工具的 JSON 转义问题。
"""
...
target = Path(__file__).parent / "simplex_core.cpp"
target.write_text(CPP_CODE, encoding="utf-8")
print(f"Written: {target} ({len(CPP_CODE)} bytes)")